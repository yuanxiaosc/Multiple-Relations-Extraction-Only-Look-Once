...
class File_Management(object):
    ...
    def file_path_and_name(self):
        ...
        predicate_head_predictions_id_file_path = os.path.join(self.MODEL_OUTPUT_DIR, "predicate_head_predictions_id.txt")
        file_path_list = [text_sentence_file_path, token_in_file_path,
                          predicate_token_label_file_path, predicate_head_predictions_id_file_path]
        file_name_list = ["text_sentence_list", "token_in_not_NUK_list ",
                          "token_label_list", "predicate_head_predictions_id"]
...
class Sorted_relation_and_entity_list_Management(File_Management):
    ...
    def get_input_list(self,):
        # text_sentence_file_path, token_in_file_path,
        # predicate_token_label_file_path, predicate_head_predictions_id_file_path
        content_list_summary, self.file_line_number = self.read_file_return_content_list()
        if len(content_list_summary) == 5:
            [text_sentence_list, token_in_not_NUK_list, token_label_list, predicate_head_predictions_id, reference_spo_list] = content_list_summary
        elif len(content_list_summary) == 4:
            [text_sentence_list, token_in_not_NUK_list, token_label_list, predicate_head_predictions_id] = content_list_summary
            reference_spo_list = [None] * len(text_sentence_list)
        else:
            raise ValueError("check code!")
        return text_sentence_list, token_in_not_NUK_list, token_label_list, predicate_head_predictions_id, reference_spo_list
    ...
    def produce_relationship_and_entity_sort_list(self):
        text_sentence_list, token_in_not_NUK_list, token_label_list, predicate_head_predictions_id_list, reference_spo_list = self.get_input_list()
        for [text_sentence, token_in_not_UNK, token_label, predicate_head_predictions_id, refer_spo_str] in\
                zip(text_sentence_list, token_in_not_NUK_list, token_label_list, predicate_head_predictions_id_list, reference_spo_list):
            text = text_sentence
            token_in_not_UNK = token_in_not_UNK.split(" ")
            token_label_out = token_label.split(" ")
            predicate_head_id_list = predicate_head_predictions_id.split(" ")
            predicate_head_id_list = [int(id) for id in predicate_head_id_list]
            predicate_head_id_matrix = np.array(predicate_head_id_list).reshape((MAX_SEQUENCE_LENGTH, MAX_SEQUENCE_LENGTH))
            print(text)
            print(token_in_not_UNK)
            print(token_label_out)
            print(predicate_head_id_matrix)
            print("\n")
    ...
    def produce_output_file(self, OUT_RESULTS_DIR=None, keep_empty_spo_list=False):
        ...
        for text, text_predicate, entity_sort_tuple_list, refer_spo_list in self.produce_relationship_and_entity_sort_list():
            ...
            subject_list = [value for value in subject_list if len(value) >= 2] #Todo:这是要求实体长度大于等于2的硬条件，未来可以删除
            object_list = [value for name, value in entity_sort_tuple_list if name == "OBJ"]
            object_list = list(set(object_list))
            object_list = [value for value in object_list if len(value) >= 2] #Todo:这是要求实体长度大于等于2的硬条件，未来可以删除
...
TEST_DATA_DIR = "bin/standard_format_data/test"
MODEL_OUTPUT_DIR = "infer_out/epochs6/ckpt0"
Competition_Mode = True
OUT_RESULTS_DIR = "infer_out/final_text_spo_list_result"
spo_list_manager = Sorted_relation_and_entity_list_Management(TEST_DATA_DIR, MODEL_OUTPUT_DIR,
                                                              Competition_Mode=Competition_Mode)
spo_list_manager.produce_relationship_and_entity_sort_list()